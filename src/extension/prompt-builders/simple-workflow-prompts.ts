...
export function buildCommitPrompt(args: string, gitContext: GitReviewContext): string {
  return [
    ...
    "",
    "Draft Conventional Commit message options for the current changes.",
    "",
    args.trim() ? `Additional instructions: ${args.trim()}` : "Additional instructions: none.",
    "",
    "Instructions:",
    "- Do not run git add, git commit, or mutate the repository.",
    "- If changes should be split into multiple commits, say so and propose the split.",
    "- Provide copy-pasteable commit message text only after explaining the grouping.",
    "",
    formatGitContext(gitContext),
  ].join("\n");
}
